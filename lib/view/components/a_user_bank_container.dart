...
class AUserBank extends StatelessWidget {
  ...
  final bool  isBankList ;

  const AUserBank({super.key, required this.themeMode, required this.isBankList});

  @override
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () {
        isBankList ? null :  navigatePush(context, const SellCryptoScreen());
      },
      child: Row(
        children: [
          Expanded(
          ...
          isBankList ?  const Padding(
            padding: EdgeInsets.only(left: 8.0),
            child: Icon(CupertinoIcons.delete, color: AppColors.kErrorPrimary,),
